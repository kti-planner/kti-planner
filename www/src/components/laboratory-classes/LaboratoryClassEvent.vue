...
const title = computed(
    () =>
        (subject ? `${makeSubjectFullName(subject, langId)} - ${makeSubjectStudyDetails(subject, langId)}\n` : '') +
...
<template>
    <div class="event-content" :title>
        ...
        <p v-if="subject" class="text-truncate fw-bold">
            {{ makeSubjectFullName(subject, langId) }} - {{ makeSubjectStudyDetails(subject, langId) }}
        </p>
