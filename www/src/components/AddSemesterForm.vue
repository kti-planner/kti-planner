...
<template>
    <div>
        <div class="d-flex justify-content-center mb-3">
            <button type="button" class="btn btn-success" @click="showForm = !showForm">
                <span v-if="!showForm">{{ translate('Add new semester') }}</span>
                <span v-else>{{ translate('Close form') }}</span>
            </button>
        </div>

        <div
            v-if="showForm"
            class="border border-secondary-subtle rounded bg-body-tertiary mx-auto p-3"
            style="max-width: 600px"
        >
            <h2 class="text-center mb-4">{{ translate('Add new semester') }}</h2>
